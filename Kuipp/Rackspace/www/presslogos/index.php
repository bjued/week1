...
<?php
	...
	while($result = mysql_fetch_assoc($query)) {
		...
		// Picture Conversion from 130x130 Profile to 60x60 Website
		//$picX = (int)$picX*60/130; // top left corner x offset (<=0)
		//$picY = (int)$picY*60/130; // top left corner y offset (<=0)
		//$picZ = $picZ*60/130; // pixel size (1 = actual pixels)
		
		// Trying clip:rect(y1,x1,y2,x2)
		/*$tlx = -1*$picX/$picZ; // normalizing the value
		$tly = -1*$picY/$picZ; // normalizing the value
		$brx = $tlx+60/$picZ;
		$bry = $tly+60/$picZ;
		echo("<div class='recent'>");
		echo("<img src='$picture' style='clip:rect($tly,$tlx,$bry,$brx)'>");
		echo("<p class='recent'>");*/
		
		/*
		 * Couldn't find a way to crop pictures in html and css
		 * Also couldn't find a way to keep the images from being
		 * distorted to the image size.
		 */
		echo("<div class='recent'>");
		echo("<img src='$picture' class='recent'");
		echo("<p class='recent'>");
