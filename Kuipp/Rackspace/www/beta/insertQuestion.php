...
QUERY;
			...
			while ($row = mysql_fetch_assoc($results)) {
				...
				if (strlen($pnDevice)>0) {
					$pn2DArray['aps'] = array("alert"=>"$pnName has just asked a question about $pnKey!", "badge"=>$row['badges']+1, "sound"=>"default");
...
QUERY;
		...
		while ($row = mysql_fetch_assoc($results)) {
			...
			$pnID = $row['userID'];
			if (strlen($pnDevice)>0) {
				$pn2DArray['aps'] = array("alert"=>"$pnName has just asked a question near you!", "badge"=>$row['badges']+1, "sound"=>"default");
				$pn2DArray['kuipp'] = array("head"=>"Question", "questionID"=>(int)$questionID);
				mysql_query("UPDATE Users SET badges=badges+1 WHERE userID='$pnID'");
				$pnNoIDs .= " AND u.userID!='$pnID'";
				include('pnQueue.php');
